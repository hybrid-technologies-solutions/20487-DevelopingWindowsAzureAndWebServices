...
namespace BlueYonder.Companion.Controllers
{
    public class FlightsController : ApiController
    {
        ...
        {
            Flights.UpdateActualDeparture(updatedSchedule, updatedSchedule.Departure);
            Flights.Save();

            updatedSchedule.FlightId = id;
            MessageHandler.Publish(updatedSchedule);
                                       ...
                                       string date = null)
        {
            ...
            var schedules = Flights.GetFlightSchedules(fromLocation, toLocation, dateObj, dateObj.AddDays(60));
        ...
        private static DateTime GetArrivalDate(DateTime date, TimeSpan duration, Location sourceLocation, Location targetLocation)
        {
            ...
            var sourceTimeZone = sourceLocation.LocationTimeZone;
            if (sourceTimeZone.IsAmbiguousTime(dateTime))
            {
                var offset = sourceTimeZone.GetAmbiguousTimeOffsets(dateTime).First();
                dateTime = dateTime.Add(offset);
            }
            else if (sourceTimeZone.IsInvalidTime(dateTime))
            {
                var delta = sourceTimeZone.GetAdjustmentRules().First().DaylightDelta;
                dateTime = dateTime.Add(delta);
            }
            return TimeZoneInfo.ConvertTime(dateTime, sourceTimeZone, targetLocation.LocationTimeZone);
        }
